/**
 * Decimal-safe currency and monetary calculation utilities in INR
 * Prevents floating point inaccuracy (e.g. 0.1 + 0.2 = 0.30000000000000004)
 */
...
export class CurrencyUtil {
   ...
   */
  public static toPaisa(rupees: number | string): bigint {
    const num = typeof rupees === 'string' ? parseFloat(rupees) : rupees;
    if (isNaN(num)) return BigInt(0);
    return BigInt(Math.round((num + Number.EPSILON) * 100));
  }

  /**
   * Convert Paisa to Rupees (number with exact 2 decimal places)
   */
  public static toRupees(paisa: bigint | number): number {
